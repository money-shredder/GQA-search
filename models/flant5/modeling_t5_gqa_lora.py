...
from .configuration_t5_lora import T5LoraConfig
...
class T5GQAttention(nn.Module):
    def __init__(self, config: T5LoraConfig, has_relative_attention_bias=False, layer_id: int = 0, groups_idx = None):
        ...
        if list_numel(self.layer_groups) != self.n_heads:
            raise ValueError(
        ...
        lora_config = config.lora_config[f"model_decoder_block_{layer_id}"]["SelfAttention"]
        ...
        self.o = LinearLora(adapter_name ="eng_alpaca", in_features = self.d_model, 
                             out_features= self.inner_dim, 
    ...
    def forward(
        ...
        kv_flops = batch_size * seq_length * (self.d_model * self.num_groups * self.n_heads + (self.d_model + self.num_groups * self.key_value_proj_dim) * self.r)
        qo_flops = batch_size * seq_length * (self.d_model ** 2 + 2 * self.d_model * self.r)
        ...
        def shape_kv(states):
            """projection"""
            return states.view(batch_size, -1, self.num_groups, self.key_value_proj_dim).transpose(1, 2)
        ...
        def project(hidden_states, proj_layer, key_value_states, past_key_value):
            ...
            if key_value_states is None:
                ...
                hidden_states = shape_kv(proj_layer(hidden_states))
            elif past_key_value is None:
                # cross-attn
                # (batch_size, n_heads, seq_length, dim_per_head)
                hidden_states = shape_kv(proj_layer(key_value_states))
        ...
        # get query states
        query_states = shape_q(self.q(hidden_states))  # (batch_size, n_heads, seq_length, dim_per_head)
        ...
        self.flops += 2 * kv_flops + qo_flops
        
        # compute scores
        scores = torch.matmul(
            query_states, key_states.transpose(3, 2)
        )  # equivalent of torch.einsum("bnqd,bnkd->bnqk", query_states, key_states), compatible with onnx op>9
        self.flops += batch_size * self.num_groups * seq_length * self.head_dim * seq_length
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(module.wo, "bias") and module.wo.bias is not None:
                module.wo.bias.data.zero_()
        elif isinstance(module, T5GQAttention):
