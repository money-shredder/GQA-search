...
import models.flant5.convert_checkpoint_t5 as cc
...
def weight_distance(model, task_name, num_labels):
    ...
    for layer_id in range(num_layers):
        ...
        for t in ("k", "v"):
            # For T5
            mat_layer = f'model.decoder.block.{layer_id}.layer.0.SelfAttention.{t}.weight'
            layer = state[mat_layer].transpose(0, 1)
...
def get_accuracy(model, gqa_model, layer, grouping, groups, t, loader, num_samples):
    ...
    state = model.state_dict()
    gqa_model.load_state_dict(cc.mha2gqa(state, tmp_grouping, num_heads=8, transpose_layer=True))
